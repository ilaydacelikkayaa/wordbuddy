...
class NetworkManager{
    ...
    func fetchDailyLesson(level:String,lessonSize:Int) async throws->[WordModel]{
    ...
    func processReview(wordId: String?, quality: Int) async throws {
        ...
        if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode != 200 {
            let statusCode = httpResponse.statusCode
            throw NSError(domain: "APIError", code: statusCode, userInfo: [NSLocalizedDescriptionKey: "SM-2 Güncelleme başarısız. Hata Kodu: \(statusCode)"])
        }
    }
